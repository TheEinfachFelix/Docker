...
def convert_to_UNIX (i):
    ...
    return((time.mktime(date_time.timetuple())*1e3 + date_time.microsecond/1e3))
...
one_higher = convert_to_UNIX(decode_data_Grid[0]["time"])+0.1
for index,wert in enumerate(decode_data_Grid):
    power_consumed_total = power_consumed_total + (abs(decode_data_Grid[index]["value"])*abs((one_higher - convert_to_UNIX(decode_data_Grid[index]["time"]))/1000)/(60*60))
    print(abs((one_higher - convert_to_UNIX(decode_data_Grid[index]["time"]))/1000)/(60*60))
    print(abs((one_higher - convert_to_UNIX(decode_data_Grid[index]["time"]))/1000)) 
